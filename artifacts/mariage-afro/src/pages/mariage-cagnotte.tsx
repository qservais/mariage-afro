...
function QrPreview({ value }: { value: string }) {
  ...
  return src ? <img src={src} alt="QR IBAN" className="w-40 h-40 border border-border bg-white p-2" /> : null;
}
      ...
      <section className="py-12 px-6 max-w-3xl mx-auto space-y-6">
        ...
        {cagnottes.map((c) => (
          <article key={c.id} className="bg-white border border-border p-6 flex flex-col md:flex-row gap-6">
            {c.photo && <img src={objectUrl(c.photo)} alt={c.title} className="w-full md:w-44 h-44 object-cover" />}
