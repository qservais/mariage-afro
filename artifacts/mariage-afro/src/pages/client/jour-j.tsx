...
export default function JourJPage() {
  ...
  return (
    <div className="space-y-6 max-w-6xl">
      <div>
        <h2 className="font-bold text-2xl">Coordination Jour J</h2>
        <p className="text-sm text-neutral-600">Timeline minute par minute, contacts d'urgence et checklist en temps réel.</p>
      </div>
      ...
      >
        ...
        <Input placeholder="Évènement" value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} required className="lg:col-span-2" data-testid="input-event-title" />
        <Input placeholder="Responsable" value={form.responsible} onChange={(e) => setForm({ ...form, responsible: e.target.value })} />
        <Button type="submit" className="rounded-none uppercase tracking-wider text-xs gap-2 lg:col-span-4"><Plus className="w-3 h-3" /> Ajouter à la timeline</Button>
      </form>

      <div className="bg-white border border-neutral-200">
        <div className="px-4 py-3 bg-background/40 text-xs uppercase tracking-widest font-bold border-b border-neutral-200">Timeline</div>
        {events.map((ev) => (
          <div key={ev.id} className="flex items-center gap-4 px-4 py-3 border-b border-neutral-100 last:border-0">
            ...
            <div className="flex-1 min-w-0">
              ...
              {ev.responsible && <p className="text-xs text-neutral-500">Responsable : {ev.responsible}</p>}
            </div>
            <button onClick={() => del.mutate(ev.id)} className="text-neutral-400 hover:text-primary"><Trash2 className="w-4 h-4" /></button>
          </div>
        ))}
        {events.length === 0 && <p className="px-4 py-8 text-center text-neutral-400 text-sm">Aucun évènement</p>}
      </div>

      <div className="bg-white border border-neutral-200">
        <div className="px-4 py-3 bg-background/40 text-xs uppercase tracking-widest font-bold border-b border-neutral-200">Contacts d'urgence</div>
        ...
        {emergencyContacts.length === 0 && <p className="px-4 py-6 text-center text-neutral-400 text-sm">Ajoutez vos prestataires avec leur téléphone pour les retrouver ici.</p>}
      </div>
    </div>
  );
}
