# PROMPT REPLIT — PRÉ-LIVRAISON COMPLÈTE — WHITE LABEL
...
9. **Footer** — liens légaux, copyright (sans crédit agence)
...
4. **Dans le récapitulatif technique final** (qui n'est PAS visible sur le site) : lister toutes les informations manquantes que le client devra fournir pour compléter les pages. Ce récapitulatif est un document technique interne, pas destiné au visiteur.
...
Pour toute question technique, contactez l'adresse email de contact du site.
```
...
Si des intégrations sont ambiguës mais probables, **ne pas les mentionner sur le site**. Les signaler uniquement dans le récapitulatif technique final (document interne).
...
## Étape 9.2 — ABSENCE TOTALE de crédit agence

**Ce site est livré en white label / sous-traitance.** Il est **strictement interdit** d'ajouter un crédit, une mention, un lien ou une signature agence dans le footer ou ailleurs sur le site.

### Interdictions absolues
- ❌ Aucune ligne "Site par [nom d'agence]" ou équivalent
- ❌ Aucun lien vers un site d'agence
- ❌ Aucun logo, favicon ou icône d'agence
- ❌ Aucun commentaire HTML mentionnant une agence (ex: `<!-- Site réalisé par ... -->`)
- ❌ Aucune mention d'agence dans les meta tags (ex: `<meta name="author">`, `<meta name="generator">`)
- ❌ Aucune référence dans les fichiers statiques (robots.txt, humans.txt, etc.)

### Ligne copyright
Le footer doit contenir **uniquement** la ligne copyright standard du client :
`© {année} {Client}. Tous droits réservés.`

Si aucune ligne copyright n'existe, en ajouter une avec le nom du client détecté dans le projet.

### Vérification white label finale
Après toutes les modifications, effectuer une recherche globale pour détecter toute trace d'agence résiduelle (commentaires, meta, assets) :

```bash
# Scanner pour toute signature d'agence possible
grep -rn -i "made by\|réalisé par\|créé par\|site par\|developed by\|designed by\|powered by" \
  --include="*.html" --include="*.tsx" --include="*.jsx" --include="*.astro" --include="*.vue" \
  --include="*.svelte" --include="*.css" --include="*.js" --include="*.ts" . \
  | grep -v node_modules | grep -v .git
# → Chaque occurrence doit être examinée et supprimée si elle mentionne une agence

# Vérifier les meta tags author/generator
grep -rn "meta.*name=[\"']author[\"']\|meta.*name=[\"']generator[\"']" \
  --include="*.html" --include="*.tsx" --include="*.jsx" --include="*.astro" \
  --include="*.vue" --include="*.svelte" .
# → Ces meta, si présents, ne doivent JAMAIS contenir de nom d'agence
```

### Validation footer
Après modification, le footer doit contenir :
- une seule occurrence de `©` maximum ;
- **aucun crédit agence** (ni visible, ni en commentaire HTML) ;
- les liens légaux ;
- aucun doublon.
...
- signaler dans le récapitulatif technique (document interne, non visible sur le site) les informations manquantes ;
...
3. **Aucun crédit agence** dans le footer, les meta, les commentaires ou assets
...
- [ ] **Aucun crédit agence** dans le footer (ligne visible)
- [ ] **Aucun crédit agence** dans les commentaires HTML, meta tags ou assets
...
- [ ] **Informations manquantes listées** (document interne, non visible sur le site)
