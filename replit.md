...
- **Branding (charte hybride officielle)** :
  - **Cœur de marque (charte PDF `WEBSITE/MARIAGE AFRO BRANDING.pdf`)** : police **Montserrat** (300 Light / 400 Regular / 500 / 600 / 700 Bold) + 4 couleurs : primary bordeaux `#68191e`, secondary cream `#fff4e4`, alt blanc `#ffffff`, alt dark `#141414`. Logo : `attached_assets/logo-mariage-affro-01.svg` (wordmark bordeaux), recoloré en blanc via `brightness-0 invert` sur fonds sombres.
  - **Extensions hors charte (validées en mode hybride)** :
    - **Cormorant Garamond** (`.font-display`/`.font-serif`) pour les titres éditoriaux (h1/h2 hero, citations, numérals décoratifs). Cohabite avec Montserrat sans la remplacer.
    - **Gold `#c9a96e`** : accent réservé aux **fonds sombres uniquement** (bg-wine-deep, dark heros, footer, cards .bg-foreground). Eyebrows, dividers, icônes, focus ring, hover accents.
    - **Gold-deep `#8a6d3b`** (4.6:1 sur cream — WCAG AA) : équivalent du gold pour les **fonds clairs** (bg-cream, bg-white). Utilisé pour eyebrows, dividers, numérals décoratifs, borders de boutons/cards, icônes sur popups blancs (ExitIntentPopup/MultiDevisForm/budget&quiz email forms).
    - **wine-deep `#1f1416`** + wine-mid `#2a1c1f` : variantes douces du dark `#141414` pour les fonds sombres riches (heros, sections premium).
  - **Règle d'usage gold** : sur cream/blanc → toujours `gold-deep` ; sur wine-deep/dark → `gold` ; jamais `gold` (bright) sur fond clair (contraste 1.94:1 KO).
